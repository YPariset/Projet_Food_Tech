...
if(isset($_POST['signup']) && $_POST['signup'] == 'done'){
    if(!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['email']) && !empty($_POST['passSign']) && !empty($_POST['streetSign']) && !empty($_POST['zipSign']) && !empty($_POST['citySign']) && !empty($_POST['birthday'])){

        $firstname = $_POST['firstname'];
        $lastname = $_POST['lastname'];
        $email = $_POST['email'];
        $password = sha1($_POST['passSign']);
        ...
        if($clientExist == 1){
            ...
            $newClient = $client->createClient($firstname, $lastname, $email, $password, $street, $zip, $city, $birthday);
